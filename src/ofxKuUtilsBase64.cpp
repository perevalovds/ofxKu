...
//--------------------------------------------------------------
std::string ofxKuUtilsBase64::decode(std::string const& encoded_string) {
	std::string chars = base64_chars();
	...
	while (in_len-- && (encoded_string[in_] != '=') && is_base64(encoded_string[in_])) {
		...
		if (i == 4) {
			for (i = 0; i < 4; i++)
				char_array_4[i] = chars.find(char_array_4[i]) & 0xff;
	...
	if (i) {
		for (j = 0; j < i; j++)
			char_array_4[j] = chars.find(char_array_4[j]) & 0xff;
